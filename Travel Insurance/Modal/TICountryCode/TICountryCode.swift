...
class TICountryCode : NSObject, NSCoding{

	var message : String!
	var response : [TICountryCodeResponse]!
	var status : Int!
	...
	@objc func parseJSONData(fromDictionary dictionary: NSDictionary)
	{
		...
		response = [TICountryCodeResponse]()
		if let responseArray = dictionary["response"] as? [NSDictionary]{
			for dic in responseArray{
				let value = TICountryCodeResponse(fromDictionary: dic)
    ...
    */
    @objc required init(coder aDecoder: NSCoder)
	{
         ...
         response = aDecoder.decodeObject(forKey: "response") as? [TICountryCodeResponse]
